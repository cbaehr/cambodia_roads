...
grid = pd.concat([grid, trt_grid[["project_id", "transactions_start_year", "end.date", "work.type", "confidence", "mrb_dist", "road_length"]]], axis=1)
...
empty_grid_out = os.path.join(base_path, "empty_grid_trimmed_REVISE.geojson")
...
del affine, roaddist, array, stats, stats_df

dst_filename=os.path.join(base_path, "REVISE_trimmed/elevation.tif")
roaddist=rasterio.open(dst_filename)
affine=roaddist.transform
array=roaddist.read(1)
stats=zonal_stats(grid, array, affine=affine, nodata=-32768)
stats_df=pd.DataFrame(stats)
stats_df.columns=["elevation" + str(i) for i in stats_df.columns]
grid=pd.concat([grid, stats_df], axis=1)

del affine, roaddist, array, stats, stats_df

dst_filename=os.path.join(base_path, "REVISE_trimmed/slope.tif")
roaddist=rasterio.open(dst_filename)
affine=roaddist.transform
array=roaddist.read(1)
stats=zonal_stats(grid, array, affine=affine, nodata=-9.99900000e+03)
stats_df=pd.DataFrame(stats)
stats_df.columns=["slope" + str(i) for i in stats_df.columns]
grid=pd.concat([grid, stats_df], axis=1)

del affine, roaddist, array, stats, stats_df

dst_filename=os.path.join(base_path, "REVISE_trimmed/ntl2000.tif")
roaddist=rasterio.open(dst_filename)
affine=roaddist.transform
array=roaddist.read(1)
stats=zonal_stats(grid, array, affine=affine, nodata=-9999)
stats_df=pd.DataFrame(stats)
stats_df.columns=["ntl2000" + str(i) for i in stats_df.columns]
grid=pd.concat([grid, stats_df], axis=1)

del affine, roaddist, array, stats, stats_df

dst_filename=os.path.join(base_path, "REVISE_trimmed/popdensity_2000.tif")
roaddist=rasterio.open(dst_filename)
affine=roaddist.transform
array=roaddist.read(1)
stats=zonal_stats(grid, array, affine=affine, nodata=-4.0764910e+14)
stats_df=pd.DataFrame(stats)
stats_df.columns=["popdensity2000" + str(i) for i in stats_df.columns]
grid=pd.concat([grid, stats_df], axis=1)

del affine, roaddist, array, stats, stats_df

dst_filename=os.path.join(base_path, "REVISE_trimmed/access_50k.tif")
roaddist=rasterio.open(dst_filename)
affine=roaddist.transform
array=roaddist.read(1)
stats=zonal_stats(grid, array, affine=affine, nodata=-2147483647)
stats_df=pd.DataFrame(stats)
stats_df.columns=["popdensity2000" + str(i) for i in stats_df.columns]
grid=pd.concat([grid, stats_df], axis=1)
...
csv_out=os.path.join(base_path, "cambodia_roads_grid_REVISE.csv")
grid.drop(["geometry"],axis=1).to_csv(csv_out, index=False)


geo_out=os.path.join(base_path, "cambodia_roads_grid_REVISE.geojson")
